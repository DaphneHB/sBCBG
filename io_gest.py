...
'''
def read_2chan_file(filename,pathToFile=os.getcwd(), model=0,ratio=None,shuffled=None) :
  results = {}
  directory = os.path.join(pathToFile,"data/files/")
  with open(os.path.join(directory,filename),"r") as varFile :
    allVardata = varFile.readlines()
  if not ("#" + str(model)) in allVardata[0] :
    print "--------- ERROR : No simulation matching this model (#"+str(model)+")"
    exit()
  if not ratio is None and not ("ratio=" + str(ratio)) in allVardata[1]:
    print "--------- ERROR : No simulation matching this ratio ("+ratio+")"
    exit()
  if not shuffled is None and not ("shuffled=" + str(shuffled)) in allVardata[2]:
    print "--------- ERROR : No simulation matching parameter : shuffle=%s" % shuffled
    exit()
  xytab = map(float,allVardata[3].split(";"))
  chan_output_dict = {}
  # reading and sadding the values to the dictionnary
  for line in allVardata[5:] :
    line = line.split(";")
...
The storing directory is "data/files/
...
'''
def write_2chan_file(xytab,trials_dico,ratio,shuffled,filename,pathToFile=os.getcwd(), model=0,antag='none') :
  print "------------- Saving data -----------"
